...
# ====================== Cloudera Credentials (prompt only once) ======================

echo "Logging into Cloudera Helm registry and preparing image pull secret..."

# Prompt for credentials only if we don't already have the secret
if ! kubectl get secret cloudera-creds -n "$CLUSTER_NAMESPACE" >/dev/null 2>&1; then
  ...
  # 1. Helm registry login (non-interactive using the captured credentials)
  echo "$CL_PASSWORD" | helm registry login container.repository.cloudera.com \
    --username "$CL_USERNAME" \
    --password-stdin

  # 2. Create the Kubernetes Docker registry secret (used by all operators for image pulls)
  ...
  # Optional: still do helm registry login if needed (in case it expired)
  # You can add a check or just let it prompt again if helm complains
fi
